...
exports.generatePdf = function(data, templatePath, fontName, extendArgs, outputFile, callback) {
    generatorXFdf(data,templatePath,outputFile,function (err) {
        if(err)
            throw err;
        let processArgs = [templatePath,'fontPath',path.dirname(__filename)+path.sep+'lib/fonts'+path.sep+fontName+'.ttf','fill_form','-', 'output','-','flatten','<'+outputFile.substring(0,outputFile.lastIndexOf('.'))+'.xfdf'+'>', outputFile];
